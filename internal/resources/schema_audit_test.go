	...
	for name, factory := range resourceFactories {
		t.Run(name, func(t *testing.T) {
			...
			if resp.Schema.Attributes == nil {
				t.Fatalf("Resource %s schema attributes is nil", name)
			}

			if _, ok := resp.Schema.Attributes["extra_config"]; !ok {
				t.Errorf("Resource %s missing required 'extra_config' attribute in schema", name)
			}
			if _, ok := resp.Schema.Attributes["region"]; !ok {
				t.Errorf("Resource %s missing required 'region' attribute in schema", name)
			}
		})
	}
}
